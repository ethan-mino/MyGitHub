...
class newLabel(pygame.sprite.Sprite):
    def __init__(self, text, fontSize, fontPath, fontColour, is_italic, xpos, ypos, background):
        pygame.sprite.Sprite.__init__(self)
        self.text = text
...
def moveSprite(sprite, x, y, centre=False, update = True):
    sprite.move(x, y, centre)
    if screenRefresh and update:
        updateDisplay()
...
def endWait():
    endLabel = makeLabel("Press ESC to quit", 25, 240, 520, "white", "./font/neodgm.ttf")  # 파라미터는 순서대로, initial text, fontsize, x, y, color, fontname, backgound를 나타낸다.
    showLabel(endLabel)
...
def makeLabel(text, fontSize, xpos, ypos, fontColour='black', font='Arial', background="clear", is_italic = False):
    # make a text sprite
    thisText = newLabel(text, fontSize, font, fontColour, is_italic, xpos, ypos, background)
    return thisText
